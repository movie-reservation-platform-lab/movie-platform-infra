...
export type CleanupReadiness = 'BLOCKED' | 'NOTHING_TO_CLEAN' | 'READY';

export type CleanupBlockerCode =
  | 'FOUNDATION_OPERATION_IN_PROGRESS'
  | 'FOUNDATION_OUTPUT_MISMATCH'
  | 'FOUNDATION_REPOSITORY_ABSENT'
  | 'WORKLOAD_STACK_PRESENT';

export type CleanupWarningCode =
  | 'FOUNDATION_TERMINATION_PROTECTION_DISABLED'
  | 'REPOSITORY_ENCRYPTION_DRIFT'
  | 'REPOSITORY_LIFECYCLE_POLICY_DRIFT'
  | 'REPOSITORY_SCAN_CONFIGURATION_DRIFT'
  | 'REPOSITORY_SCAN_CONFIGURATION_UNKNOWN'
  | 'REPOSITORY_TAG_MUTABILITY_DRIFT'
  | 'REPOSITORY_TAG_MUTABILITY_EXCLUSIONS_PRESENT'
  | 'REPOSITORY_TAGS_DRIFT'
  | 'RETAINED_REPOSITORY_WITHOUT_STACK';
